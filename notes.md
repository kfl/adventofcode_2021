...
* Added a brute-force solution that explicitly keeps track of all
  possible states based on input that can only be 1-9. Current
  solution find the minimal model no in 20 mins, which is not as bad
  as I initially expected, but now I think it should be possible to do
  faster.

* Added a backtracking depth-first interpreter, where we use a crude
  value range analysis for pruning impossible paths. This finds the
  minimal modelno in ~25s which is almost as fast as Z3, and it runs
  in constant memory (4K).
